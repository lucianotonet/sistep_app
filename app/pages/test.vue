<template>
  <main class="flex flex-1 flex-col items-center justify-center leading-tight select-none">
    <transition name="fade" mode="out-in">
      <div v-if="currentQuestion < questions.length" :key="'question-' + currentQuestion"
        ...
        <p class="mb-4 text-xs text-muted-foreground">Progresso: {{ currentQuestion + 1 }} / {{ questions.length }}</p>
      ...
      <div v-else :key="'results'" class="flex flex-col text-center">
        <p class="mb-4">Questionário concluído!</p>
        <p class="mb-2">Resultado:</p>
        <p>Depressão: {{ results.depression }}</p>
        <p>Ansiedade: {{ results.anxiety }}</p>
        <p>Estresse: {{ results.stress }}</p>
        <br />
...
const options = ['Nunca', 'Às vezes', 'Frequentemente', 'Quase sempre'];
